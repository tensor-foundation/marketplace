...
function getAuctionHouseFromOpts(auctionHouse, walletKeyPair, tMintKey) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (auctionHouse) {
            ...
            auctionHouseKey = (yield (0, accounts_1.getAuctionHouse)(walletKeyPair.publicKey, tMintKey))[0];
        }
...
programCommand('show_escrow')
    ...
    const anchorProgram = yield (0, accounts_1.loadAuctionHouseProgram)(walletKeyPair, env);
    const auctionHouseKey = new anchor_1.web3.PublicKey(auctionHouse);
    const auctionHouseObj = yield anchorProgram.account.auctionHouse.fetch(auctionHouseKey);
    if (!otherWallet) {
        loglevel_1.default.info('No --wallet passed in, defaulting to keypair');
    }
    const escrow = (yield (0, accounts_1.getAuctionHouseBuyerEscrow)(auctionHouseKey, otherWallet || walletKeyPair.publicKey))[0];
    const amount = yield (0, accounts_1.getTokenAmount)(anchorProgram, escrow, 
    //@ts-ignore
...
programCommand('withdraw')
    ...
    const anchorProgram = yield (0, accounts_1.loadAuctionHouseProgram)(walletKeyPair, env);
    const auctionHouseObj = yield anchorProgram.account.auctionHouse.fetch(auctionHouseKey);
    const amountAdjusted = yield (0, various_1.getPriceWithMantissa)(amount, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram);
    const [escrowPaymentAccount, bump] = yield (0, accounts_1.getAuctionHouseBuyerEscrow)(auctionHouseKey, walletKeyPair.publicKey);
    //@ts-ignore
    const isNative = auctionHouseObj.treasuryMint.equals(constants_1.WRAPPED_SOL_MINT);
    const ata = (yield (0, accounts_1.getAtaForMint)(
    //@ts-ignore
    auctionHouseObj.treasuryMint, walletKeyPair.publicKey))[0];
    const signers = [];
    const currBal = yield (0, accounts_1.getTokenAmount)(anchorProgram, escrowPaymentAccount, 
    //@ts-ignore
    auctionHouseObj.treasuryMint);
    const instruction = yield anchorProgram.instruction.withdraw(bump, new anchor_1.BN(amountAdjusted), {
        accounts: {
    ...
    instruction.keys
        ...
        .map(k => (k.isSigner = true));
    yield (0, transactions_1.sendTransactionWithRetryWithKeypair)(anchorProgram.provider.connection, walletKeyPair, [instruction], signers, 'max');
...
programCommand('sell')
    ...
    const auctionHouseKeypairLoaded = auctionHouseKeypair
        ? (0, accounts_1.loadWalletKey)(auctionHouseKeypair)
        : null;
    const anchorProgram = yield (0, accounts_1.loadAuctionHouseProgram)(auctionHouseSigns ? auctionHouseKeypairLoaded : walletKeyPair, env);
    const auctionHouseObj = yield anchorProgram.account.auctionHouse.fetch(auctionHouseKey);
    const buyPriceAdjusted = new anchor_1.BN(yield (0, various_1.getPriceWithMantissa)(buyPrice, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram));
    const tokenSizeAdjusted = new anchor_1.BN(yield (0, various_1.getPriceWithMantissa)(tokenSize, mintKey, walletKeyPair, anchorProgram));
    const tokenAccountKey = (yield (0, accounts_1.getAtaForMint)(mintKey, walletKeyPair.publicKey))[0];
    const [programAsSigner, programAsSignerBump] = yield (0, accounts_1.getAuctionHouseProgramAsSigner)();
    // const metadata = await getMetadata(mintKey);
    const [tradeState, tradeBump] = yield (0, accounts_1.getAuctionHouseTradeState)(auctionHouseKey, walletKeyPair.publicKey, tokenAccountKey, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, mintKey, tokenSizeAdjusted, buyPriceAdjusted);
    const [freeTradeState, freeTradeBump] = yield (0, accounts_1.getAuctionHouseTradeState)(auctionHouseKey, walletKeyPair.publicKey, tokenAccountKey, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, mintKey, tokenSizeAdjusted, new anchor_1.BN(0));
    const signers = [];
    const instruction = yield anchorProgram.instruction.sell(tradeBump, freeTradeBump, programAsSignerBump, buyPriceAdjusted, tokenSizeAdjusted, {
        accounts: {
            wallet: walletKeyPair.publicKey,
            metadata: yield (0, accounts_1.getMetadata)(mintKey),
    ...
    yield (0, transactions_1.sendTransactionWithRetryWithKeypair)(anchorProgram.provider.connection, auctionHouseSigns ? auctionHouseKeypairLoaded : walletKeyPair, [instruction], signers, 'max');
...
programCommand('withdraw_from_treasury')
    ...
    const auctionHouseKey = yield getAuctionHouseFromOpts(auctionHouse, walletKeyPair, tMintKey);
    const auctionHouseObj = yield anchorProgram.account.auctionHouse.fetch(auctionHouseKey);
    const amountAdjusted = new anchor_1.BN(yield (0, various_1.getPriceWithMantissa)(amount, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram));
    const signers = [];
    const instruction = yield anchorProgram.instruction.withdrawFromTreasury(amountAdjusted, {
        accounts: {
            //@ts-ignore
            treasuryMint: auctionHouseObj.treasuryMint,
            //@ts-ignore
...
programCommand('withdraw_from_fees')
    ...
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram));
    const signers = [];
    const instruction = yield anchorProgram.instruction.withdrawFromFee(amountAdjusted, {
        accounts: {
            //@ts-ignore
            authority: auctionHouseObj.authority,
...
programCommand('cancel')
    ...
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram));
    const tokenSizeAdjusted = new anchor_1.BN(yield (0, various_1.getPriceWithMantissa)(tokenSize, mintKey, walletKeyPair, anchorProgram));
    const results = yield anchorProgram.provider.connection
    ...
    const tradeState = (yield (0, accounts_1.getAuctionHouseTradeState)(auctionHouseKey, walletKeyPair.publicKey, tokenAccountKey, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, mintKey, tokenSizeAdjusted, buyPriceAdjusted))[0];
    const signers = [];
    const instruction = yield anchorProgram.instruction.cancel(buyPriceAdjusted, tokenSizeAdjusted, {
        accounts: {
            wallet: walletKeyPair.publicKey,
            tokenAccount: tokenAccountKey,
            tokenMint: mintKey,
...
programCommand('execute_sale')
    ...
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram));
    const tokenSizeAdjusted = new anchor_1.BN(yield (0, various_1.getPriceWithMantissa)(tokenSize, mintKey, walletKeyPair, anchorProgram));
    const tokenAccountKey = (yield (0, accounts_1.getAtaForMint)(mintKey, sellerWalletKey))[0];
    const buyerTradeState = (yield (0, accounts_1.getAuctionHouseTradeState)(auctionHouseKey, buyerWalletKey, tokenAccountKey, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, mintKey, tokenSizeAdjusted, buyPriceAdjusted))[0];
    const sellerTradeState = (yield (0, accounts_1.getAuctionHouseTradeState)(auctionHouseKey, sellerWalletKey, tokenAccountKey, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, mintKey, tokenSizeAdjusted, buyPriceAdjusted))[0];
    const [freeTradeState, freeTradeStateBump] = yield (0, accounts_1.getAuctionHouseTradeState)(auctionHouseKey, sellerWalletKey, tokenAccountKey, 
    //@ts-ignore
    auctionHouseObj.treasuryMint, mintKey, tokenSizeAdjusted, new anchor_1.BN(0));
    const [escrowPaymentAccount, bump] = yield (0, accounts_1.getAuctionHouseBuyerEscrow)(auctionHouseKey, buyerWalletKey);
    const [programAsSigner, programAsSignerBump] = yield (0, accounts_1.getAuctionHouseProgramAsSigner)();
    const metadata = yield (0, accounts_1.getMetadata)(mintKey);
    const metadataObj = yield anchorProgram.provider.connection.getAccountInfo(metadata);
    ...
    for (let i = 0; i < metadataDecoded.data.creators.length; i++) {
        ...
        if (!isNative) {
            remainingAccounts.push({
                pubkey: (yield (0, accounts_1.getAtaForMint)(
                //@ts-ignore
    ...
    const instruction = yield anchorProgram.instruction.executeSale(bump, freeTradeStateBump, programAsSignerBump, buyPriceAdjusted, tokenSizeAdjusted, {
        accounts: {
            ...
            sellerPaymentReceiptAccount: isNative
                ? sellerWalletKey
                : (yield (0, accounts_1.getAtaForMint)(tMint, sellerWalletKey))[0],
            buyerReceiptTokenAccount: (yield (0, accounts_1.getAtaForMint)(mintKey, buyerWalletKey))[0],
            //@ts-ignore
            authority: auctionHouseObj.authority,
            auctionHouse: auctionHouseKey,
            //@ts-ignore
...
programCommand('buy')
    ...
    auctionHouseObj.treasuryMint, walletKeyPair, anchorProgram));
    const tokenSizeAdjusted = new anchor_1.BN(yield (0, various_1.getPriceWithMantissa)(tokenSize, mintKey, walletKeyPair, anchorProgram));
    const [escrowPaymentAccount, escrowBump] = yield (0, accounts_1.getAuctionHouseBuyerEscrow)(auctionHouseKey, walletKeyPair.publicKey);
    const results = yield anchorProgram.provider.connection.getTokenLargestAccounts(mintKey);
    ...
    const instruction = yield anchorProgram.instruction.buy(tradeBump, escrowBump, buyPriceAdjusted, tokenSizeAdjusted, {
        accounts: {
            wallet: walletKeyPair.publicKey,
    ...
    yield (0, transactions_1.sendTransactionWithRetryWithKeypair)(anchorProgram.provider.connection, walletKeyPair, instructions, signers, 'max');
...
programCommand('deposit')
    ...
    const transferAuthority = anchor_1.web3.Keypair.generate();
    const signers = isNative ? [] : [transferAuthority];
    const instruction = yield anchorProgram.instruction.deposit(bump, new anchor_1.BN(amountAdjusted), {
        accounts: {
            wallet: walletKeyPair.publicKey,
            paymentAccount: isNative ? walletKeyPair.publicKey : ata,
            transferAuthority: isNative
...
programCommand('show')
    ...
    const auctionHouseKey = yield getAuctionHouseFromOpts(auctionHouse, walletKeyPair, tMintKey);
    const auctionHouseObj = yield anchorProgram.account.auctionHouse.fetch(auctionHouseKey);
    const treasuryAmount = yield (0, accounts_1.getTokenAmount)(anchorProgram, 
    //@ts-ignore
    auctionHouseObj.auctionHouseTreasury, 
    //@ts-ignore
    auctionHouseObj.treasuryMint);
    const feeAmount = yield anchorProgram.provider.connection.getBalance(
    //@ts-ignore
...
programCommand('create_auction_house')
    ...
    const twdAta = tMintKey.equals(constants_1.WRAPPED_SOL_MINT)
        ? twdKey
        : (yield (0, accounts_1.getAtaForMint)(tMintKey, twdKey))[0];
    const [auctionHouse, bump] = yield (0, accounts_1.getAuctionHouse)(walletKeyPair.publicKey, tMintKey);
    const [feeAccount, feeBump] = yield (0, accounts_1.getAuctionHouseFeeAcct)(auctionHouse);
    const [treasuryAccount, treasuryBump] = yield (0, accounts_1.getAuctionHouseTreasuryAcct)(auctionHouse);
    yield anchorProgram.rpc.createAuctionHouse(bump, feeBump, treasuryBump, sfbp, requiresSignOff == 'true', canChangeSalePrice == 'true', {
        accounts: {
...
programCommand('update_auction_house')
    ...
    if (!treasuryWithdrawalDestination) {
        ...
        twdKey = tMintKey.equals(constants_1.WRAPPED_SOL_MINT)
            ? //@ts-ignore
                auctionHouseObj.treasuryWithdrawalDestination
            : (0, accounts_1.deserializeAccount)(Buffer.from((yield anchorProgram.provider.connection.getAccountInfo(
            //@ts-ignore
    ...
    yield anchorProgram.rpc.updateAuctionHouse(sfbp, rso, ccsp, {
        accounts: {
            treasuryMint: tMintKey,
            payer: walletKeyPair.publicKey,
            authority: walletKeyPair.publicKey,
...
function programCommand(name) {
    return commander_1.program
        .command(name)
        .option('-e, --env <string>', 'Solana cluster env name, i.e. mainnet-beta, testnet, devnet', 'devnet')
