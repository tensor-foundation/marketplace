...
test('fees are paid correctly', async (t) => {
  ...
  const { buyer, nftOwner, nftUpdateAuthority } = signers;
  const { mint, extraAccountMetas, distribution, sellerFeeBasisPoints } = nft;
...
// TODO -- determine how we're handling maker brokers in Marketplace.
test.skip('maker and taker brokers receive correct split', async (t) => {
  const {
    client,
    signers,
    nft,
