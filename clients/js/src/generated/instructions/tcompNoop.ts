...
export const TCOMP_NOOP_DISCRIMINATOR = new Uint8Array([
  106, 162, 10, 226, 132, 68, 223, 21,
]);

export function getTcompNoopDiscriminatorBytes() {
  return fixEncoderSize(getBytesEncoder(), 8).encode(TCOMP_NOOP_DISCRIMINATOR);
}
...
export function getTcompNoopInstructionDataEncoder(): Encoder<TcompNoopInstructionDataArgs> {
  return transformEncoder(
    ...
    (value) => ({ ...value, discriminator: TCOMP_NOOP_DISCRIMINATOR })
  );
}
...
>(
...
): TcompNoopInstruction<TProgramAddress, TAccountTcompSigner> {
  // Program address.
  ...
  } as TcompNoopInstruction<TProgramAddress, TAccountTcompSigner>;
