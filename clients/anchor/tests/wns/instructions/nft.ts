...
export const getAddNftToGroupIx = async (
  provider: Provider,
  args: AddGroupArgs
) => {
  const metadataProgram = getMetadataProgram(provider);
  ...
  const ix = await metadataProgram.methods
    ...
    .accountsStrict({
      ...
      systemProgram: SystemProgram.programId,
      tokenProgram: tokenProgramId,
      group: new PublicKey(args.group),
...
export const getAddRoyaltiesIx = async (
  provider: Provider,
  args: AddRoyaltiesArgs
) => {
  const metadataProgram = getMetadataProgram(provider);
...
export const getFreezeNftIx = async (
  provider: Provider,
  args: FreezeNftArgs
) => {
  const metadataProgram = getMetadataProgram(provider);
