...
export const getNftTransferApproveIx = async (
  provider: Provider,
  args: ApproveTransferNftArgs
  ...
  const ix = await metadataProgram.methods
    ...
    .accounts({
      ...
      distributionTokenAccount: getAtaAddress(
        args.paymentMint,
        getDistributionAccountPda(args.groupMint, args.paymentMint).toString()
      ),
      authorityTokenAccount: getAtaAddress(args.mint, args.authority),
      distributionAccount: getDistributionAccountPda(
        args.groupMint,
        args.paymentMint
      ),
...
export const getNftTransferIx = async (args: TransferNftArgs) => {
  ...
  transferIx.keys = transferIx.keys.concat([
    // System program
    {
      pubkey: getApproveAccountPda(args.mint),
      isSigner: false,
      isWritable: true,
    },
    ...
    {
      pubkey: getExtraMetasAccountPda(args.mint),
      isSigner: false,
      isWritable: false,
    },
  ]);
